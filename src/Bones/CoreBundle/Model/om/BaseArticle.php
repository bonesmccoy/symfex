...
abstract class BaseArticle extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'Bones\\CoreBundle\\Model\\ArticlePeer';
    ...
    /**
     * The value for the meta_columns field.
     * @var        string
     */
    protected $meta_columns;
    ...
    /**
     * Get the [meta_columns] column value.
     *
     * @return string
     */
    ...
    /**
     * Set the value of [meta_columns] column.
     *
     * @param  string $v new value
     * @return Article The current object (for fluent API support)
     */
    public function setMetaColumns($v)
    {
        if ($v !== null) {
            $v = (string) $v;
        }

        if ($this->meta_columns !== $v) {
            $this->meta_columns = $v;
            $this->modifiedColumns[] = ArticlePeer::META_COLUMNS;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            $this->meta_columns = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
            $this->resetModified();
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(ArticlePeer::META_COLUMNS)) {
            $modifiedColumns[':p' . $index++]  = '`meta_columns`';
        }

        $sql = sprintf(
            'INSERT INTO `bones_article` (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case '`content`':
                        ...
                        break;
                    case '`meta_columns`':
                        $stmt->bindValue($identifier, $this->meta_columns, PDO::PARAM_STR);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[3], $arr)) $this->setMetaColumns($arr[$keys[3]]);
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ArticlePeer::META_COLUMNS)) $criteria->add(ArticlePeer::META_COLUMNS, $this->meta_columns);
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setMetaColumns($this->getMetaColumns());
        if ($makeNew) {
            $copyObj->setNew(true);
