 ...
 * @method ArticleQuery orderByMetaColumns($order = Criteria::ASC) Order by the meta_columns column
 *
 ...
 * @method ArticleQuery groupByMetaColumns() Group by the meta_columns column
 *
 ...
 * @method Article findOneByMetaColumns(string $meta_columns) Return the first Article filtered by the meta_columns column
 *
 ...
 * @method array findByMetaColumns(string $meta_columns) Return Article objects filtered by the meta_columns column
 */
abstract class BaseArticleQuery extends ModelCriteria
{
    ...
    public function __construct($dbName = null, $modelName = null, $modelAlias = null)
    {
        ...
        if (null === $modelName) {
            $modelName = 'Bones\\CoreBundle\\Model\\Article';
        }
    ...
    {
        $sql = 'SELECT `id`, `title`, `content`, `meta_columns` FROM `bones_article` WHERE `id` = :p0';
        try {
    ...
    /**
     * Filter the query on the meta_columns column
     *
     * Example usage:
     * <code>
     * $query->filterByMetaColumns('fooValue');   // WHERE meta_columns = 'fooValue'
     * $query->filterByMetaColumns('%fooValue%'); // WHERE meta_columns LIKE '%fooValue%'
     * </code>
     *
     * @param     string $metaColumns The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ArticleQuery The current query, for fluid interface
     */
    public function filterByMetaColumns($metaColumns = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($metaColumns)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $metaColumns)) {
                $metaColumns = str_replace('*', '%', $metaColumns);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(ArticlePeer::META_COLUMNS, $metaColumns, $comparison);
    }
